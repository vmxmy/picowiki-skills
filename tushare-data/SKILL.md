...
若缺失 token，**不要继续执行任何数据查询**，立即向用户索要：

> Tushare Token 未配置。请提供你的 Token（在 [tushare.pro](https://tushare.pro/register) 注册后获取）：

用户提供后，用以下方式写入本地并验证：

```python
import tushare as ts
ts.set_token('用户提供的token')   # 写入 ~/.tushare/token
pro = ts.pro_api()
pro.trade_cal(exchange='SSE', start_date='20240101', end_date='20240103')  # 冒烟测试
```

冒烟通过后再继续原始任务。若冒烟失败，提示 token 无效并再次询问。

Token 检测顺序：
1. `os.getenv('TUSHARE_TOKEN')` — 环境变量
2. `ts.get_token()` — 本地文件 `~/.tushare/token`
3. 以上均无 → 主动向用户提问

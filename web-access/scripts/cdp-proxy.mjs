...
async function getWebSocketUrl(port, wsPath) {
  ...
  // 从 /json/version 获取带 UUID 的真实 WebSocket URL
  try {
    const res = await fetch(`http://127.0.0.1:${port}/json/version`);
    const data = await res.json();
    if (data.webSocketDebuggerUrl) {
      const url = data.webSocketDebuggerUrl.replace('localhost', '127.0.0.1');
      console.log(`[CDP Proxy] 从 /json/version 获取 WS URL: ${url}`);
      return url;
    }
  } catch (e) {
    console.warn('[CDP Proxy] 无法从 /json/version 获取 WS URL，回退默认路径');
  }
